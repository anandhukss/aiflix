...
function MovieInfo({ closeDialog, movie }) {
  ...
  useEffect(() => {
    const url = `${CORE_URL}/${movie.id}/videos`;
    fetch(url, API_OPTIONS)
      .then((res) => res.json())
      .then((json) => setVideoList(json.results))
  ...
  if (!videoList)
    return (
      <div role="status">
        <svg
          aria-hidden="true"
          className="inline w-20 h-20 text-white animate-spin dark:text-gray-600 fill-primary-bg"
          viewBox="0 0 100 101"
          fill="none"
      ...
      <div className="w-full rounded-md p-4 bg-primary-bg shadow">
        <div className="flex justify-end">
          <button className="cursor-pointer" onClick={closeDialog}>
            <i
              className="fa fa-close text-red-500 fa-lg"
              aria-hidden="true"
            ></i>
          </button>
        </div>
